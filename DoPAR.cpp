...
void DoPAR::transformDMs(vector<vector<size_color> >& listXY, vector<vector<size_color> >& listXZ, vector<vector<size_color> >& listYZ) {
	...
	printf("Solid_Upper=%d,Pore_Upper=%d,Pore_Lower=%d", Solid_Upper[MULTIRES - 1], Pore_Upper[MULTIRES - 1], Pore_Lower[MULTIRES - 1]);
}
...
void DoPAR::equalizeHistograms(int level, vector<vector<size_color>>& TIsXY, vector<vector<size_color>>& TIsXZ, vector<vector<size_color>>& TIsYZ) {
	...
	int maxv(0);
	for (int n = 0; n < MultiTIsNum; n++) {
		maxv = max(maxv, (int)*max_element(TIsXY[n].begin(), TIsXY[n].end()));
		if (!SIM2D_YN) maxv = max(maxv, (int)max(*max_element(TIsXZ[n].begin(), TIsXZ[n].end()), *max_element(TIsYZ[n].begin(), TIsYZ[n].end())));
	...
	cout << " maxv= " << maxv;
	double scale;
	scale = min(255, maxv)*1.0 / total;
	Pore_Upper[level] = min(255, maxv);

	// Build LUT from accumulate histrogram
	vector<uchar> lut(maxv+1, 0);
	long sum = 0;
	for (int i = 0; i <= maxv; ++i) {
		sum += hist[i];
		//round((sum - hist[0])/(total-hist[0])*(bins-1));			//here hist[0]=0		
		lut[i] = min(Pore_Upper[level], (uchar)round(sum * scale));		// the value is saturated in range [0, max_val]
	}
...
void DoPAR::initColorHis_exemplar() {
	for (int level = 0; level < MULTIRES; level++) {
		...
		cout << endl << "actualBinNum at level"<< level <<" = " << actualBinNum;
